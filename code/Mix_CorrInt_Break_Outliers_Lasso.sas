/* t̩l̩chargement sur SAS du module Iman-Conover transformation */
proc iml;
...
Do t=1 to 1000 ; 
	...
	/* IMAN - CONOVER : MIX CORR & OUTLIERS */

	C = toeplitz({1, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55}); /* Matrice de correlation de toeplitz*/  

	load module = ImanConoverTransform;
	W = ImanConoverTransform(X[,1:7], C); 
	...
	do m=1 to 100; 
	if m<=50 then y[m] = X1[m, 1:7]*beta1 + eps[m]; 
	else y[m] = X1[m, 1:7]*beta2 +eps[m]; 
	end; 
...
proc sgplot data=nouveau_long;
    ...
    title "Intern Correlation & Structural Break & Outliers Lasso (Stop = SBC)";
run;
