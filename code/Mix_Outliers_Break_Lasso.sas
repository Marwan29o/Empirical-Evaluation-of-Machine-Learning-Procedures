...
Do t=1 to 1000 ; 
	...
	/* On doit tirer dans une loi uniforme compris entre 0 et 1*/ 
	/* Si u<= 0.9 --> Dans la première loi N1
	   Sinon --> Dans la deuxième loi N2
	mu2 > mu1
	*/ 
	...
	do j=1 to ncol(X); 
	u=rand('UNIFORM');
	if u <= 0.9	then X[i,j] = rand("normal", 0, 1); 
	else X[i,j] = rand("normal", 4,1);
	end; end; 

	eps = normal(j(TT,1,1))*0.1; /* bruit gaussien sur 100 observations*/ 


	beta1 = {-0.7, 0.8, 0.4, 0.67, 0.77, -0.25, 0.44}; /* beta avant rupture*/ 

	beta2 = {0.67, -0.25, 0.8, -0.25, 0.77, -0.7, 0.4}; /* beta après rupture*/ 
...
data Lasso_freq2;
    ...
    ID = _N_; /* _N_ génère un compteur qui s'incré©mente de 1 pour chaque ligne */
run;
...
proc sgplot data=nouveau_long;
    ...
    title "Structural Break & Outliers Lasso (Stop = SBC)";
run;
