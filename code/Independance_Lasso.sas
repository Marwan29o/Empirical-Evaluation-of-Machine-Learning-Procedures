...
proc iml ; 
mu = j(50, 1, 0); /* On créer le vecteur mu qui correspond à la moyenne de toutes les variables qui sont indépendantes */ 

vcov = I(50); /* Matrice identité d'ordre 50 --> Indépendance entre les variables 
qui sont par ailleurs réduites (variance de 1) */ 
...
Do t=1 to 1000 ; 
	...
	X = randnormal(TT, Mu, vcov); /* On génère une loi normale sur 100 observations */

	eps = normal(j(TT,1,1))*0.1; /* bruit gaussien sur 100 observations*/ 

	beta = {-0.7, 0.8, 0.4, 0.67, 0.77, -0.25, 0.44}; /* On définit les coefficients de notre modèle*/ 

	y = X[,1:7]*beta+eps; /* On crée le modèle
	On a ici un modèle qui a les vrais variables
	càd seulement 7 sur les 50 que nous avons généré*/
 
	table = y||X; /* On crée une table qui concatène y (variable expliquée) et X (les 50 variables explicatives)*/ 

	name = 'y' // ('X1' : 'X50')`; /* On crée les labels de chaque colonne*/ 
...
submit; 
	...
	*print ensemble;
	...
	/*Under*/ 
	if ncol(ensemble)>1 & ncol(ensemble)<8 & nrow(selection)=ncol(ensemble) then underfitting = underfitting +1 ; /* crÃ©er un ensemble autre qui contient les var 8 Ã  50 => FALSE */
	else underfitting = underfitting ;

	*Wrong
	if ncol(ensemble)=1 then wrongmodel=wrongmodel +1 ; 
	*else wrongmodel = wrongmodel ;
	end;
...
proc sgplot data=nouveau_long;
    ...
    title "Independence Lasso (Stop = SBC)";
run;
