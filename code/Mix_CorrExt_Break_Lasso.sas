...
proc iml ; 
mu1 = j(15, 1, 0); /* On créer le vecteur mu1 qui a 15 lignes et qui est rempli de 0 qui correspond à la moyenne des variables corrélées */ 

vcov1 = toeplitz({1, 0.95, 0.90, 0.85, 0.8, 0.75, 0.70, 0.65, 0.60, 0.55, 0.50, 0.45, 0.40, 0.35, 0.3}); 

/*On a injecté de la correlation sur nos 15 premières variables afin d'avoir de la correlation externe */

mu2 = j(35, 1, 0); /* On créer le vecteur mu2 qui a 35 lignes et qui est rempli de 0 qui correspond à la moyenne des variables restantes qui ne sont pas corrélées */

vcov2 = I(35); /* On a une la matrice de variance covariance d'identité d'odre 35 : des 35 variables non corrélées */
...
Do t=1 to 1000 ; 
	...
	/*Rupture : on créer 2 vecteur des coefficients, beta1 : avant et beta2 : après */

	beta1 = {-0.7, 0.8, 0.4, 0.67, 0.77, -0.25, 0.44}; 
	beta2 = {0.67, -0.25, 0.8, -0.25, 0.77, -0.7, 0.4};
	...
	do m=1 to nrow(X); 
	if m<=50 then y[m] = X[m, 1:7]*beta1 + eps[m]; 
	else y[m] = X[m, 1:7]*beta2 +eps[m]; 
	end; 
...
proc sgplot data=nouveau_long;
    ...
    title "Extern Correlation & Structural Break Lasso (Stop = SBC)";
run;
